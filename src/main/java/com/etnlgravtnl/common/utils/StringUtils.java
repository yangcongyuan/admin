...
public class StringUtils extends org.apache.commons.lang3.StringUtils {
	 ...
	 * 获得用户远程地址
	 */
	public static String getRemoteAddr(ContainerRequestContext requestContext){
		String remoteAddr = requestContext.getHeaderString("X-Real-IP");
        if (isNotBlank(remoteAddr)) {
        	remoteAddr = requestContext.getHeaderString("X-Forwarded-For");
        }else if (isNotBlank(remoteAddr)) {
        	remoteAddr = requestContext.getHeaderString("Proxy-Client-IP");
        }else if (isNotBlank(remoteAddr)) {
        	remoteAddr = requestContext.getHeaderString("WL-Proxy-Client-IP");
        }
        return remoteAddr ;
	}
